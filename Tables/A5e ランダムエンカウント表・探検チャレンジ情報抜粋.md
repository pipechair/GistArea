...
### 結果(Outcomes)
各探検チャレンジには、いくつかの可能な結果がリストアップされている。これらの結果は、クリティカル失敗からクリティカル成功までの4つのカテゴリーに分けられている（「[[#探検チャレンジ：成功と失敗(Exploration Challenges: Success and Failure)]]」を参照）。冒険家たちがどのような行動を取るかによって、どの結果になるかは完全にナレーターの裁量に委ねられている。独創的な解決策は即座にクリティカル成功となるかもしれないし、全員が成功するグループ・チェックも同様である。また、悲惨な決断がクリティカル失敗を引き起こすこともあるが、最終的に探検チャレンジの結果と、付与される報酬や発生するペナルティはゲームマスターが決定する。
